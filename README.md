...
参考   
[syzkaller/docs/linux/setup_ubuntu-host_qemu-vm_x86-64-kernel.md](https://github.com/google/syzkaller/blob/master/docs/linux/setup_ubuntu-host_qemu-vm_x86-64-kernel.md)  
[Debugging kernel and modules via gdb](https://www.kernel.org/doc/html/latest/dev-tools/gdb-kernel-debugging.html)

图省事取消了securityFS，另外选择了buster（debian10）新版本而非文档中的stretch

* 运行`create-image.sh`创建image
* debug.sh 运行虚拟机(-s -S表示调试)
* 进入到`/path/build/linux` 目录，再执行  
    `gdb vmlinux -ex 'target remote:1234' -ex 'b start_kernel' -ex 'c'`  
    (因为kernel在`vmlinux-gdb.py`中提供了lx-xxx一系列辅助调试工具，比如`p $lx_current().pid`可以很方便获取current宏，进入目录可以自动加载python脚本)


此外，文档只提交 `git add create-image.sh debug.sh ssh-conn.sh README.md` ，其余文件是`create-image.sh`运行之后创建的。
